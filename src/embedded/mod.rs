...
pub fn run_embedded(mut config: runtime::Config) -> Result<Client, Error> {
    trace!("Citadel runtime configuration: {:#?}", &config);
    config.process();
    trace!("Processed configuration: {:#?}", &config);

    debug!("Starting Citadel runtime in background");
    ...
    thread::spawn(move || {
        runtime::run(config).expect("Error in Citadel runtime")
    });
